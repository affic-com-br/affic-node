...
No `--provenance` here: provenance can only be generated by a supported CI with OIDC, and the
command fails outside one. Every later release goes through the workflow, which does attach it.
