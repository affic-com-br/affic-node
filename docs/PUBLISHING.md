...
`--no-provenance` is not optional here. `publishConfig.provenance` in `package.json` is `true`, so
npm tries to attach a provenance statement on every publish — and it can only build one inside a
supported CI with OIDC. From a laptop it fails with
`Automatic provenance generation not supported for provider: null`. The flag turns it off for this
one manual publish; releases from the workflow still get provenance.

npm will also ask for a one-time password and print a browser URL to authenticate. Complete it in
the browser, and the publish resumes on its own.
